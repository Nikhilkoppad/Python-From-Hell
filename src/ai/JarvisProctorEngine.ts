...
export class JarvisProctorEngine {
  ...
  public static observe(observation: ProctorObservation | LegacyObservation): ProctorResponse {
    ...
    if (failuresThisAttempt >= 3) return this.response('angry', "Three failures on the same problem. Stop smashing Run like it's a fucking slot machine. We're debugging the actual misconception now.", true, true, false, true, false, false);
    if (runtimeError || errorType) return this.response('thinking', "The interpreter just told us exactly where you screwed up. Let's read the traceback instead of panicking.", true, false, true, true, false, false);
