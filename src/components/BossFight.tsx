...
interface BossFightProps {
  ...
  onVictory: (xpEarned: number, bossId: string) => void;
  onClose: () => void;
}
  ...
  const handleExecuteAttack = async () => {
    ...
    try {
      ...
      if (allPassed) {
        ...
        const firstClear = !(progress.clearedBosses ?? []).includes(boss.id);
        setRewardAwarded(firstClear);
        onVictory(firstClear ? 150 : 0, boss.id);
      } else {
      ...
      <div className="max-w-4xl w-full bg-slate-950 border-2 border-red-600 rounded-2xl shadow-[0_0_100px_rgba(220,38,38,0.5)] flex flex-col max-h-[92vh] overflow-hidden">
        <div className="p-6 bg-gradient-to-r from-red-950 via-slate-950 to-black border-b border-red-900/80 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="text-4xl p-2 bg-red-900/30 border border-red-600 rounded-xl animate-pulse">{boss.bossAvatar}</div>
            <div><div className="flex items-center gap-2"><h2 className="text-xl font-black text-white tracking-wider">{boss.bossName}</h2><span className="text-[10px] px-2 py-0.5 rounded bg-red-950 border border-red-700 text-red-400 font-bold uppercase">BOSS ARENA</span></div><p className="text-xs text-red-400 font-bold">{boss.bossTitle}</p></div>
          </div>
        ...
        <div className="p-4 bg-slate-900/90 border-t border-slate-800 flex justify-between items-center">
          <button onClick={onClose} className="px-4 py-2.5 text-xs text-slate-400 hover:text-white font-bold rounded-lg hover:bg-slate-800 transition-colors">← RETREAT</button>
          ...
          {battleState === 'VICTORY' && <div className="flex items-center gap-4"><span className="text-xs text-emerald-400 font-bold flex items-center gap-1.5"><Award className="w-4 h-4" /> {rewardAwarded ? 'VICTORY! +150 XP EARNED' : 'VICTORY! BOSS ALREADY CLEARED — NO DOUBLE XP'}</span><button onClick={onClose} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-black font-black text-xs uppercase tracking-widest rounded-xl transition-all">CLAIM SPOILS & RETURN →</button></div>}
