...
interface BossFightProps { progress: UserProgress; language: LearningLanguage | string; onVictory: (xpEarned: number, bossId: string) => void; onClose: () => void; }
...
const BOSS_CHALLENGES: BossChallenge[] = [{
  ...
  starterCode: `def sanitize_command(cmd_str):
    # Cerberus intentionally starts broken. Repair the logic.
    if "DROP" in cmd_str.upper():
        return "DENIED"
    return cmd_str
`,
  tests: [
...
export const BossFight: React.FC<BossFightProps> = ({ progress, language, onVictory, onClose }) => {
  ...
  const startBattle = () => { setBattleState('ACTIVE'); setBossHp(boss.maxHp); setTimeLeft(boss.timeLimitSeconds); setRewardAwarded(false); setCombatLog([isHindi ? '🔥 BATTLE START: Cerberus jaag chuka hai! Code likh ke attack kar!' : '🔥 BATTLE INITIATED: Cerberus awakened. Slay the demon before the timer expires!']); };
  ...
  const handleExecuteAttack = async () => {
    ...
    try {
      ...
      if (allPassed) {
        ...
        const clearKey = `python-from-hell:boss-cleared:${boss.id}`;
        const firstClear = !(progress.clearedBosses ?? []).includes(boss.id) && localStorage.getItem(clearKey) !== '1';
        if (firstClear) localStorage.setItem(clearKey, '1');
