...
interface ProjectFactoryProps {
  ...
  onCompleteProject: (projectId: string, xpEarned: number) => void;
  onClose: () => void;
}

export const ProjectFactory: React.FC<ProjectFactoryProps> = ({
  progress,
  language,
  onCompleteProject,
  onClose,
  ...
  // Multi-file state: Map filename -> code content
  const [fileContents, setFileContents] = useState<Record<string, string>>(() =>
    Object.fromEntries(project.files.map((f) => [f.name, f.initialContent]))
  );
  ...
  const handleConsultDirector = async () => {
    ...
    try {
      const response = await AITeacherService.requestGuidance({
        ...
        lessonConcept: activeMilestone.title,
        userCode: fileContents[activeFileName] || '',
        userQuery: `As the AI Project Director, review my implementation of ${activeMilestone.title} in project ${project.title}. Give concise engineering critique.`,
        mode: 'EXPLAIN',
      });
      ...
      setAiCritique(
        isHindi
          ? 'Director: Code structure ko clean rakh aur requirements follow kar.'
          : 'Director: Ensure strict type validation and clean modular structure before verifying.'
      );
    } finally {
  ...
  const handleVerifyMilestone = async () => {
    ...
    try {
      ...
      // Bundle all files together to support multi-file imports in the Pyodide runtime
      // Replace import statements with inlined code definitions for seamless worker execution
      let bundledExecutionScript = '';
      
      // Add models.py and ledger.py contents first
      if (fileContents['models.py']) {
        bundledExecutionScript += `# --- models.py ---\n${fileContents['models.py']}\n\n`;
      }
      if (fileContents['ledger.py']) {
        // Strip out 'from models import Expense' since models is already defined above
      ...
      if (fileContents['main.py']) {
        const cleanMain = fileContents['main.py']
          .replace(/from ledger import ExpenseLedger/g, '# (imported above)')
          .replace(/from models import Expense/g, '# (imported above)');
        bundledExecutionScript += `# --- main.py ---\n${cleanMain}\n\n`;
      }

      // Append the milestone test harness
      const cleanTestHarness = activeMilestone.testHarness
        .replace(/from models import Expense/g, '# (imported above)')
        .replace(/from ledger import ExpenseLedger/g, '# (imported above)');
      ...
      if (isPassed) {
        ...
        setDirectorFeedback({
          passed: true,
          message: activeMilestone.directorFeedbackOnSuccess,
          actualOutput,
        });

        // If last milestone, trigger project completion
        if (currentMilestoneIdx === project.milestones.length - 1) {
          onCompleteProject(project.id, 100);
        } else {
          // Advance to next milestone after short delay
          setTimeout(() => {
            setCurrentMilestoneIdx((prev) => prev + 1);
            setDirectorFeedback(null);
          }, 1200);
        }
      } else {
        setDirectorFeedback({
          passed: false,
          message: activeMilestone.directorFeedbackOnFailure,
          actualOutput: actualOutput || 'No output generated / Runtime error',
        });
      }
    } catch {
      setDirectorFeedback({
        passed: false,
        message: 'Runtime failed to execute project bundle.',
      });
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4 font-mono">
      <div className="max-w-6xl w-full bg-slate-950 border-2 border-red-600/70 rounded-2xl shadow-[0_0_100px_rgba(220,38,38,0.4)] flex flex-col max-h-[94vh] overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-black text-white tracking-wider">PROJECT DIRECTOR // FACTORY</h2>
                <span className="text-[10px] px-2 py-0.5 rounded bg-red-950 border border-red-800 text-red-400 font-bold uppercase">
                  MULTI-FILE ARCHITECTURE
                </span>
              </div>
              <p className="text-xs text-slate-400">
                {isHindi ? 'Real-world multi-file Python architecture build karo aur progressive milestones verify karo.' : 'Real-world software engineering with milestone validation.'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 flex-1 overflow-y-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* LEFT: PROJECT DIRECTOR & MILESTONE BOARD */}
          <div className="lg:col-span-5 space-y-4 flex flex-col">
            ...
            <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl space-y-3">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center justify-between">
                ...
                <span className="text-emerald-400">{completedMilestones.length} / {project.milestones.length} CLEARED</span>
              </div>
                  ...
                  return (
                    ...
                    >
                      {isDone ? (
                        ...
                        <span className="w-4 h-4 rounded-full border border-slate-700 flex items-center justify-center text-[10px] flex-shrink-0 mt-0.5">
                          {idx + 1}
                        </span>
                      )}
                      <div>
                        <div>{m.title}</div>
                        {isCurrent && <div className="text-[10px] text-slate-400 font-normal mt-1">{m.objective}</div>}
                      </div>
                    </div>
                  );
                })}
            ...
            {/* ACTIVE MILESTONE BRIEF & AI DIRECTOR REVIEW */}
            <div className="p-4 bg-slate-900/90 border border-slate-800 rounded-xl space-y-3 flex-1">
              <div className="flex items-center justify-between">
                <div className="text-[10px] text-red-400 font-bold uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  <span>PROJECT DIRECTOR CONSULTATION</span>
                </div>
                <button
                  onClick={handleConsultDirector}
                  disabled={isAiConsulting}
                  className="px-3 py-1 bg-red-600/20 border border-red-500/50 hover:bg-red-600/40 text-red-300 text-[10px] font-bold rounded-lg transition-all flex items-center gap-1"
                >
                  <Sparkles className="w-3 h-3" />
                  <span>{isHindi ? 'Director Feedback' : 'Request Architecture Review'}</span>
                </button>
              </div>

              {isAiConsulting ? (
                <div className="text-xs text-slate-400 animate-pulse py-2">Director is reviewing code structure...</div>
              ) : aiCritique ? (
                ...
                <div className="text-xs text-slate-500 py-2">
                  Follow the milestone requirements in the file tabs and run verification when ready.
                </div>
              )}
            </div>
          ...
          {/* RIGHT: MULTI-FILE CODE EDITOR & TEST HARNESS */}
          <div className="lg:col-span-7 space-y-4 flex flex-col">
            
            {/* FILE TABS */}
            <div className="flex items-center justify-between bg-slate-900 px-4 pt-3 rounded-t-xl border-t border-x border-slate-800">
              ...
              >
                ...
                <span>{isVerifying ? 'VERIFYING...' : (isHindi ? 'MILESTONE TEST KAR' : 'VERIFY MILESTONE')}</span>
              </button>
            </div>

            {/* ACTIVE FILE EDITOR */}
            <div className="flex-1 bg-black border-x border-b border-slate-800 rounded-b-xl p-4 flex flex-col space-y-2">
              <textarea
                value={fileContents[activeFileName] || ''}
                onChange={(e) => {
                  const updatedVal = e.target.value;
                  setFileContents((prev) => ({ ...prev, [activeFileName]: updatedVal }));
                }}
                className="w-full flex-1 min-h-[300px] bg-black font-mono text-xs text-green-400 focus:outline-none resize-none leading-relaxed"
                spellCheck={false}
              />
            </div>

            {/* DIRECTOR VERIFICATION FEEDBACK */}
            {directorFeedback && (
              <div
                className={`p-4 rounded-xl border text-xs space-y-2 ${
                  directorFeedback.passed
              ...
              >
                <div className="flex items-center justify-between font-bold">
                  <span className="flex items-center gap-1.5">
                    {directorFeedback.passed ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : <XCircle className="w-4 h-4 text-red-400" />}
                    <span>{directorFeedback.passed ? 'MILESTONE PASSED & APPROVED' : 'MILESTONE REJECTED'}</span>
                  </span>
                </div>

                <p className="leading-relaxed">{directorFeedback.message}</p>

                {directorFeedback.actualOutput && (
                  <div className="text-[10px] text-slate-400 font-mono mt-1">
                    Output: {directorFeedback.actualOutput}
                  </div>
                )}
              </div>
            )}
            ...
            {completedMilestones.length === project.milestones.length && (
              <div className="p-4 bg-emerald-950/60 border border-emerald-600 rounded-xl text-center text-xs font-bold text-emerald-300 flex items-center justify-center gap-2">
                <Award className="w-5 h-5 text-yellow-400" />
                <span>PROJECT COMPLETE: HADES EXPENSE LEDGER SHIPPED! (+100 XP EARNED)</span>
              </div>
            )}
