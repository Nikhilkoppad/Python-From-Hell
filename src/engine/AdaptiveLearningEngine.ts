...
type LegacyAttemptInput = { profile: LearningProfile; challengeId: string; lessonId: string; topicId: string; passed: boolean; hintsUsed: number; code?: string; output?: string; runtimeError?: string; errorType?: string };
...
export class AdaptiveLearningEngine {
  public static recordAttempt(input: LegacyAttemptInput): { profile: LearningProfile; decision: any } {
    ...
    const challengeType = this.inferChallengeType(input.challengeId, existing);
    const updatedSkill = SkillMasteryEngine.recordAttempt(existing, { passed: input.passed, challengeType, hintsUsed: input.hintsUsed, errorType: input.errorType, independent: input.hintsUsed === 0 });
    skills[input.topicId] = updatedSkill;
    const history = [...((profile.attemptHistory ?? []) as Array<AttemptHistoryEntry & { challengeId?: string; lessonId?: string; independent?: boolean }>), { challengeId: input.challengeId, lessonId: input.lessonId, topicId: input.topicId, skillId: input.topicId, passed: input.passed, hintsUsed: input.hintsUsed, errorType: input.errorType, timestamp: Date.now(), independent: input.hintsUsed === 0 }];
    const successes = history.filter((attempt) => attempt.passed).length;
    const nextProfile: LearningProfile = { ...profile, skills, attemptHistory: history, totalSuccesses: successes, totalFailures: history.length - successes, successfulAttempts: successes, independentSolves: history.filter((attempt) => attempt.passed && attempt.independent).length, totalHintsUsed: history.reduce((sum, attempt) => sum + Number(attempt.hintsUsed ?? 0), 0), overallMastery: this.calculateOverallMastery(skills), currentPhase: input.passed ? 'PRACTICE' : 'DEBUG', topicMastery: { ...(profile.topicMastery ?? {}), [input.topicId]: updatedSkill.mastery }, topicAccuracy: { ...(profile.topicAccuracy ?? {}), [input.topicId]: updatedSkill.evidence.attempts ? Math.round(updatedSkill.evidence.successes / updatedSkill.evidence.attempts * 100) : 0 }, hintDependency: { ...(profile.hintDependency ?? {}), [input.topicId]: updatedSkill.evidence.attempts ? updatedSkill.evidence.hintsUsed / updatedSkill.evidence.attempts : 0 }, weakTopics: Object.values(skills).filter((skill: Skill) => skill.weak).map((skill: Skill) => skill.id), masteredTopics: Object.values(skills).filter((skill: Skill) => skill.masteryLevel === 'MASTERED').map((skill: Skill) => skill.id), currentTopicId: input.topicId, recentMistakes: input.passed ? (profile.recentMistakes ?? []) : [...(profile.recentMistakes ?? []), { challengeId: input.challengeId, topicId: input.topicId, skillId: input.topicId, errorType: input.errorType, error: input.runtimeError, timestamp: Date.now() }].slice(-20) };
  ...
  public static decideNextAction(_profile: LearningProfile, skill: Skill, passed: boolean, challengeType: ChallengeType, difficulty: number, hintsUsedThisAttempt: number, errorType?: string): AdaptiveDecision {
    ...
    if (!passed) {
      if (this.countRecentSameErrors(evidence.recentErrors, errorType) >= 2) return { action: 'MICRO_LESSON', reason: 'Repeated misconception detected. Reteach the concept.', skillId: skill.id, recommendedDifficulty: Math.max(1, difficulty - 1), removeHints: false };
    ...
    const accuracy = this.calculateAccuracy(evidence); const independence = this.calculateIndependence(evidence);
    if (hintsUsedThisAttempt >= 2) return { action: 'INDEPENDENT_CHALLENGE', reason: 'Passed with significant assistance. Require independent proof.', skillId: skill.id, recommendedDifficulty: difficulty, removeHints: true };
  ...
  public static isBossReady(skill: Skill, profile: LearningProfile): boolean { const e = skill.evidence; return skill.unlocked && !(profile.weakTopics ?? []).includes(skill.id) && e.attempts >= 4 && this.calculateAccuracy(e) >= 0.8 && this.calculateIndependence(e) >= 0.75 && this.countEvidenceTypes(e) >= 2; }
