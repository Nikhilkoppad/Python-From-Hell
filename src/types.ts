...
/** Shared shell contract. Optionality is intentional while the adaptive model migrates persisted legacy records. */
export interface UserProgress {
  id?: string; userId?: string; xp: number; streak: number; level: number; currentLessonId: string; currentChallengeIndex: number;
  completedLessons: string[]; masteredTopics?: string[]; weakTopics?: string[]; roastIntensity: RoastIntensity | string; learningLanguage: LearningLanguage | string;
  diagnosticCompleted: boolean; incidents?: IncidentLog[] | unknown[]; achievements?: any[];
  topicStats?: Record<string, TopicAssessment>; topicMastery?: Record<string, number>; topicAccuracy?: Record<string, number>; topicRetention?: Record<string, number>;
  topicIndependentSolve?: Record<string, number>; hintDependency?: Record<string, number>; confidenceIndicators?: Record<string, number>; lastActiveTimestamp?: string | number;
  interviewReadiness: number; recommendations?: Recommendation[] | unknown[]; routingLogs?: AIRoutingRecord[] | unknown[]; aiTutor?: any; behavioralPatterns?: any; misconceptions?: any;
}
