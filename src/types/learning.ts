...
>>>>>>> 620f1740be78eda26853a878796da3d4abfe3826
...
<<<<<<< HEAD
export interface ChallengeEvaluation { passed?: boolean; score?: number; outputCorrect?: boolean; conceptCorrect?: boolean; errorType?: string; errorMessage?: string; feedback?: string; fixHint?: string; roast?: string; skillImpact?: { skillId: string; delta: number }[]; mastery?: any; masteryLevel?: any; accuracy?: any; independence?: any; consistency?: any; challengeVariety?: any; hintDependency?: any; weak?: any; readyForBoss?: any; }
...
export interface AttemptHistoryEntry { challengeId: string; lessonId?: string; topicId: string; skillId?: string; passed: boolean; hintsUsed: number; independent?: boolean; runtimeError?: string; errorType?: string; code?: string; output?: string; timestamp: number; timeSpentSeconds?: number; }
export interface AdaptiveDecision { action: 'CONTINUE' | 'TEACH_AGAIN' | 'MICRO_LESSON' | 'EASIER_CHALLENGE' | 'HARDER_CHALLENGE' | 'DEBUG_CHALLENGE' | 'INDEPENDENT_CHALLENGE' | 'BOSS_CHALLENGE' | 'DEBUG' | 'INDEPENDENT_RETRY' | 'ADVANCE' | 'TEACH' | 'PRACTICE'; reason: string; skillId: string; topicId?: string; phase?: LearningPhase; confidence?: number; recommendedDifficulty: number; removeHints: boolean; }
export interface LearningProfile {
  studentId: string; overallMastery: number; currentPhase: LearningPhase; totalSuccesses: number; totalFailures: number; skills: Record<string, Skill>;
  misconceptions: { tracked: string[]; lastDetected: string; detectionCount: number };
  xp?: number; totalAttempts?: number; roastIntensity?: string; learningLanguage?: string; currentChallengeIndex?: number; completedLessons?: string[]; diagnosticCompleted?: boolean; incidents?: any[]; achievements?: any[]; interviewReadiness?: number;
  recommendations?: any[]; routingLogs?: any[]; topicStats?: Record<string, any>; topicMastery?: Record<string, number>; topicAccuracy?: Record<string, number>; topicRetention?: Record<string, number>; topicIndependentSolve?: Record<string, number>; hintDependency?: Record<string, number>; confidenceIndicators?: Record<string, number>;
  ...
  behavioralPatterns?: any; aiTutor?: any; lastDecision?: AdaptiveDecision;
}
=======
