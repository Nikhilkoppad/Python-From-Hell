...
export interface AttemptHistoryEntry { topicId?: string; skillId?: string; passed: boolean; hintsUsed: number; errorType?: string; timestamp: number; timeSpentSeconds?: number; challengeId?: string; lessonId?: string; independent?: boolean; }

/** Transitional learner contract: both the canonical skill model and legacy UI fields are supported. */
export interface LearningProfile {
  ...
  xp?: number; level?: number; streak?: number; currentSkill?: any; assistedSolves?: number; totalAttempts?: number; successfulAttempts?: number; independentSolves?: number; totalHintsUsed?: number;
  currentTopicId?: string; currentChallengeIndex?: number; currentLessonId?: string; completedLessons?: string[]; roastIntensity?: string; learningLanguage?: string; diagnosticCompleted?: boolean;
  ...
  lastActiveTimestamp?: number | string; lastDecision?: any;
}

export interface AdaptiveDecision { action: 'CONTINUE' | 'TEACH_AGAIN' | 'MICRO_LESSON' | 'EASIER_CHALLENGE' | 'HARDER_CHALLENGE' | 'DEBUG_CHALLENGE' | 'INDEPENDENT_CHALLENGE' | 'BOSS_CHALLENGE'; reason: string; skillId: string; recommendedDifficulty: number; removeHints: boolean; }
